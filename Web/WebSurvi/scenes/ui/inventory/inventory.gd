...
func _unhandled_input(_event):
	...
	if Input.is_action_pressed("chat"):
		if is_instance_valid(chatinput):
			if chatinput.text:
				player.sendMessage(chatinput.text)
			chatinput.queue_free()
		else:
...
func populateRecipes():
	...
	for recipe in Items.recipes.keys():
		...
		recipeSlot.canCraft = Inventory.canCraftItem("1", recipe)
...
func itemSelected(id):
	if id in Items.equips.keys():
		player.player_items.tryEquipItem(id)
	elif player.player_items.equippedItem:
		player.player_items.unequipItem()
		
	var consumeList := Items.consume.keys()
	if id in consumeList:
		player.player_items.consumeItem(id, Items.consume[id] ) 
...
func recipeSelected(id):
	selectedRecipe = id
	if Inventory.canCraftItem("1", id):
	...
	for ing in Items.recipes[id].keys():
		...
		itemSlot.itemId = ing
		itemSlot.setRecipeText(Inventory.checkItemCount("1", ing),Items.recipes[id][ing])
...
func _on_startcraft_button_pressed():
	Inventory.tryCraftItem("1", selectedRecipe)
