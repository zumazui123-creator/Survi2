...
func punchCheckCollision():
	if spawnsProjectile:
		sendProjectile(player.player_movement.direction)
...
func objectDestroyed():
	increaseScore(Constants.OBJECT_SCORE_GAIN)

func mobKilled():
	increaseScore(Constants.MOB_SCORE_GAIN)

func enemyPlayerKilled():
	increaseScore(Constants.PK_SCORE_GAIN)
...
func die():
	...
	Multihelper._deregister_character(peerId) # für Server
	player.player_items.dropInventory()
	Multihelper.showSpawnUI()
	player.queue_free()
